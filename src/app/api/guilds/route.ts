...
// POST /api/guilds — cadastra uma guild inimiga (só admin, mesma regra de
// POST /api/enemies): baixa o elenco inteiro do site e importa cada membro
// como inimigo (ver `src/lib/guild-import.ts`).
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await importEnemyGuild(guildId, session.user.id);
    return NextResponse.json(result, { status: 201 });
  } catch (err) {
    console.error("[GuildImport]", err);
